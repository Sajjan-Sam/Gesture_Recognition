# ==========================================================
# MULTI-MODEL + GRU (FINAL WITH TEST DISPLAY + BEST SAVE)
# ==========================================================
...
RESULT_DIR = "per_gru_results_all_models"
MODEL_DIR = "per_gru_models_all_models"
GRAPH_DIR = "per_gru_graphs_all_models"
...
class CNN_LSTM(nn.Module):
    def __init__(self, backbone, lstm_layers):
        ...
        # CHANGED: LSTM → GRU
        self.gru = nn.GRU(self.feat_dim, 256,
...
for model_name in models:
    for train_combo in train_sets:
        for lstm_layers in lstm_layers_list:
            ...
            name = f"{model_name}_{'_'.join(train_combo)}_gru{lstm_layers}"

            print(f"\nModel={model_name} | Train={train_combo} | GRU={lstm_layers}")
            ...
            val_acc, history = train_model(model, train_loader, val_loader, save_path)
...
df.to_csv(os.path.join(RESULT_DIR, "results_gru.csv"), index=False)

plt.figure(figsize=(14,6))
sns.barplot(data=df, x="Train_Set", y="Test_Acc", hue="Model")
plt.xticks(rotation=45)
plt.tight_layout()
plt.savefig(os.path.join(GRAPH_DIR, "final_comparison_gru.png"))
